...
# lidarDataLog = sio.loadmat('./lidar_images_sphere/Lidar_Image_Double_Loop_Sphere_1.mat')
lidarDataLog = sio.loadmat('./lidar_images_sphere_cov/Lidar_Image_Double_Loop_Sphere_1.mat')

## Fetch the number of double loop files
# trajectory_files = glob.glob("./lidar_images_sphere/*.mat")
trajectory_files = glob.glob("./lidar_images_sphere_cov/*.mat")
...
lidarSphere = lidarDataLog['log']['lidarSphere'][0][0]
lidarSphere = torch.tensor(np.float32(lidarSphere))
...
lidarSphereLog = torch.zeros(tuple(torch.cat((torch.tensor([num_train]), torch.tensor(lidarSphere.shape)), axis=0)))
...
for file_idx in range(num_train):
    ...
    lidarSphereLog[file_idx,:,:,:] = lidarSphere
...
numLidarImg = lidarSphere.shape[0]
...
class Lidar_CNN_Pose_Cov(nn.Module):
    def __init__(self):
        ...
        # Define constants
        self.image_size = np.uint16([60, 360])
        self.num_fc_input = int(5040)
...
# lidarCNNPose.load_state_dict(torch.load('./model/lidarCNN_Pano_L1_Flatten_Batch10.pth'))
# # Transfer the state dict over to the new model
# lidarCNNPoseCov.en_conv1 = lidarCNNPose.en_conv1
# lidarCNNPoseCov.en_conv2 = lidarCNNPose.en_conv2
# lidarCNNPoseCov.en_conv3 = lidarCNNPose.en_conv3
# lidarCNNPoseCov.fc1_pose = lidarCNNPose.fc1
# lidarCNNPoseCov.fc2_pose = lidarCNNPose.fc2
# lidarCNNPoseCov.fc3_pose = lidarCNNPose.fc3
# lidarCNNPoseCov.out_pose = lidarCNNPose.out_pose
# torch.save(lidarCNNPoseCov.state_dict(), './model/lidarCNNPoseCov_Pose_Trained.pth')
...
if (training_enable == True):
    ...
    for epoch in range(0,num_epoch+1):
        ...
        for img_idx in train_idx:
            ...
            input_tensor = lidarSphereLog[:, img_idx, :, :, :]
        ...
        if epoch % 250 == 0:
            ...
            torch.save(lidarCNNPoseCov.state_dict(), './model/spherical/'+file_name+'.pth')
...
for file_idx in range(num_files):
    ...
    for img_idx in range(numLidarImg):
        ...
        input_tensor = lidarSphere[img_idx, :, :].unsqueeze(0)
...
# savemat("lidarCNN_Sph_Pose_Train_30Lap.mat", workspace_var)
savemat("lidarCNN_Sph_PoseCov_Train_30Lap.mat", workspace_var)
